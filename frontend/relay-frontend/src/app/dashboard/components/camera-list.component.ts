...
@Component({
  ...
  template: `<section class="grid"><article *ngFor="let c of cameras" class="card"><h3><a [routerLink]="['/camera']" [fragment]="c.id">{{c.name}}</a></h3><p><b>URI:</b> <code>{{c.uri}}</code></p><p><b>Host:</b> {{c.host}}:{{c.port}}</p><p class="state">Status variable / possibly offline</p><a [routerLink]="['/camera']" [fragment]="c.id">Open controls →</a></article></section>`,
