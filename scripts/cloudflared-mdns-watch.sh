...
# Auto-generated by cloudflared-mdns-watch.sh
# Source mDNS: ${MDNS_NAME}
tunnel: ${TUNNEL_UUID}
...
ingress:
  ...
  - hostname: ${HOSTNAME_STATUS}
    service: http://${ip}:${PORT}/status
  - hostname: ${HOSTNAME_STREAM}
    service: http://${ip}:${PORT}/stream
...
while true; do
  ...
  if ! healthcheck "$current_ip"; then
    echo "WARN: healthcheck failed for ${current_ip}, forcing config rewrite + restart"
    changed=1
  fi

  if [[ "$changed" -eq 1 ]]; then
    write_config "$current_ip"
